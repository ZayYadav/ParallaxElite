...
public class ProxyService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        return AppServiceDispatcher.get().onStartCommand(intent, flags);
    }
    ...
    private void showNotification() {
        NotificationCompat.Builder builder = new NotificationCompat.Builder(getApplicationContext(), getPackageName() + ".ParallaxELiteCore_Proxy").setPriority(NotificationCompat.PRIORITY_MAX);
        if (BuildCompat.isOreo()) {
            startForeground(ParallaxELiteInstaller.getHostPkg().hashCode(), builder.build());
        }
    }
