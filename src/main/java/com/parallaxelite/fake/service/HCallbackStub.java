    ...
    private Boolean handleLaunchActivity(Object client) {
        ...
        if (activityInfo != null) {
            if (BActivityThread.getAppConfig() == null) {
                ParallaxELiteInstaller.getBActivityManager().restartProcess(
                        activityInfo.packageName, activityInfo.processName, stubRecord.mUserId);
                Intent originalTarget = stubRecord.mTarget;
                if (originalTarget == null) {
                    originalTarget = ParallaxELiteInstaller.getBPackageManager()
            ...
            ParallaxELiteInstaller.getBActivityManager().onActivityCreated(taskId, token, stubRecord.mActivityRecord);

            if (BuildCompat.isS()) {
                Object record = BRActivityThread.get(ParallaxELiteInstaller.mainThread()).getLaunchingActivity(token);
                if (record != null) {
    ...
    private boolean handleCreateService(Object data) {
        if (BActivityThread.getAppConfig() != null) {
            ...
            if (!serviceInfo.name.equals(ProxyManifest.getProxyService(BActivityThread.getAppPid()))
                ...
                ParallaxELiteInstaller.getBActivityManager().startService(intent, null, false, BActivityThread.getUserId());
                return true;
            }
        }
        return false;
