...
public class ContentProviderDelegate {
    ...
    public static void update(Object holder, String auth) {
        ...
        switch (auth) {
            case "settings":
                bContentProvider = new SystemProviderStub().wrapper(iInterface, ParallaxELiteInstaller.getHostPkg());
                break;
            default:
                bContentProvider = new ContentProviderStub().wrapper(iInterface, ParallaxELiteInstaller.getHostPkg());
                break;
        }
        if (BuildCompat.isOreo()) {
    ...
    public static void init() {
        ...
        ParallaxELiteInstaller.getContext().getContentResolver().call(Uri.parse("content://settings"), "", null, null);
        Object activityThread = ParallaxELiteInstaller.mainThread();
        ...
        for (Object value : map.values()) {
            ...
            if (!sInjected.contains(providerName)) {
                ...
                BRActivityThreadProviderClientRecordP.get(value)._set_mProvider(new ContentProviderStub().wrapper(iInterface, ParallaxELiteInstaller.getHostPkg()));
